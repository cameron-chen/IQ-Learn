...
os.environ["PYTHONPATH"] = "/home/zichang/proj/IQ-Learn/iq_learn"
import argparse
...
def run_exp(config, datafile):
    ...
    if config.get("env") == "cheetah":
        ...
        full_loader = utils_hil.humanoid_full_loader(100)
        hssm.post_obs_state._output_normal = True
        hssm._output_normal = True
    elif config.get("env") == "cheetah_long":
        full_loader = utils_hil.cheetah_long_full_loader(100)
        hssm.post_obs_state._output_normal = True
        hssm._output_normal = True
    else:
        raise ValueError()
...
def render_skills(config, emb_list, exp_name):
    ...
    if config.get("env") == "hil":
        full_loader = utils_hil.hil_full_loader(100)
    else:
        raise ValueError()
