...
class HumanoidDataset(Dataset):
    def __init__(self, partition, seq_size=1000):
        mycwd = os.getcwd()
        os.chdir("/home/zichang/proj/IQ-Learn/iq_learn/")
        dataset_paths = ['/home/zichang/proj/IQ-Learn/iq_learn/experts/Humanoid-v2_25.pkl']
        state = []
        action = []
        level = []
        for idx, dataset_path in enumerate(dataset_paths):
...
def humanoid_full_loader(batch_size):
    full_dataset = HumanoidDataset(partition="full")
    full_loader = DataLoader(
        dataset=full_dataset, batch_size=batch_size, shuffle=False
    )
    return full_loader

class CheetahLongDataset(Dataset):
    def __init__(self, partition, seq_size=1000):
        mycwd = os.getcwd()
        os.chdir("/home/zichang/proj/IQ-Learn/iq_learn/")
    ...
    def open_dataset(self, dataset_path):
        ...
        print('action', len(action), action[0].shape)
        return state, action
    
def cheetah_long_full_loader(batch_size):
    full_dataset = CheetahLongDataset(partition="full")
    full_loader = DataLoader(
        dataset=full_dataset, batch_size=batch_size, shuffle=False
    )
    return full_loader